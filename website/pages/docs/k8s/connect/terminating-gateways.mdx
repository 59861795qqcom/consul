...
```shell-session
$ export CONSUL_HTTP_TOKEN=$(kubectl get secret <consul-release>-bootstrap-acl-token -o jsonpath={.data.token} | base64 -D)
...
$ consul acl policy update -name terminating-gateway-terminating-gateway-token -rules @write-policy.hcl \
-merge-policies -merge-roles -merge-service-identities
$ consul acl policy update -name terminating-gateway-terminating-gateway-token -rules @write-policy.hcl
```
...
-> You can verify connectivity of the application via a curl command:
```shell-session
